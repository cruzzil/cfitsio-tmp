...
{ 
	...
	/* deal with special cases.  Note that the variable length array case */
	/* must be tested first, because the field in the table holds the     */
	/* descriptors, whatever the data type they point to;  this must      */
	/* match the equivalent tests in fits_compress_table.                 */
	if (coltype[ii] < 0) {  /* pointer to variable length array */
	        if (colcode[ii] == 'P')
