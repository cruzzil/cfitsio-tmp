...
int	ngp_read_xtension(fitsfile *ff, int parent_hn, int simple_mode)
 { int		r, exflg, l, my_hn, tmp0, incrementor_index, i, j;
   ...
   if (NGP_OK != (r = ngp_hdu_init(&ngph))) return(r);
						...
						/* every error exit from here on must */
						/* release the tokens read so far */
   if (NGP_OK != (r = ngp_read_line(0)))	/* EOF always means error here */
     { ngp_hdu_clear(&ngph); return(r); }
   switch (NGP_XTENSION_SIMPLE & simple_mode)
     {
       case 0:  if (NGP_TOKEN_XTENSION != ngp_keyidx)
		  { ngp_hdu_clear(&ngph); return(NGP_TOKEN_NOT_EXPECT); }
		break;
   ...
   if (NGP_OK == r)
     { if (parent_hn > 0)
         { fits_get_hdu_num(ff, &my_hn);
           ...
           if (NGP_OK != r) { ngp_hdu_clear(&ngph); return(r); }
         }
     }
